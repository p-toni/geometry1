...
export function createParticleScroll(
  ...
  if (!glOrNull || glOrNull.isContextLost()) return null;
  const gl = glOrNull;
  ...
  let bg: [number, number, number] = [0.957, 0.945, 0.918];
...
export interface ParticleScrollProps extends ParticleScrollOptions {
  ...
  className?: string;
  style?: CSSProperties;
  onReady?: (api: ParticleScrollInstance) => void;
}
...
export function ParticleScroll({
  ...
  useEffect(() => {
    ...
    content.scrollTop = 0;
    const api = createParticleScroll(
      { source, content, output },
      initialOptions,
    );
    instanceRef.current = api;
    if (native && !api) setFailed(true);
    if (api) onReadyRef.current?.(api);
    return () => {
      api?.destroy();
    ...
    >
      ...
      >
        {native ? (
          <div
            ref={contentRef}
            className="particle-scroll-content"
            data-home-scroller
            style={contentStyle}
          >
            {children}
